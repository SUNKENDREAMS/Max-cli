...
The integration tests are designed to validate the end-to-end functionality of Max Headroom. They execute the built binary in a controlled environment and verify that it behaves as expected when interacting with the file system and local AI models (e.g., Ollama, if tests are adapted for it).
...
For more detailed debugging, the `--verbose` flag streams the real-time output from the `max-headroom` command to the console.
...
... output from the max-headroom command ...
...
The workflow runs the tests in different sandboxing environments (if sandboxing is maintained) to ensure Max Headroom is tested across each:
